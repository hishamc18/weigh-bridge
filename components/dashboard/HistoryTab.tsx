...
import { Pencil, X, Check, Trash2, ChevronLeft, ChevronRight, Search } from 'lucide-react';
...
export default function HistoryTab() {
  ...
  const filtered = useMemo(() => {
    ...
    return bills.filter((b) => {
      ...
      if (fromDate) {
        if (new Date(b.printDate) < new Date(fromDate)) return false;
      }
      if (toDate) {
  ...
  async function handleSave() {
    ...
    try {
      const shopId =
        typeof editForm.shop === 'object' ? (editForm.shop as ShopOption)._id : editForm.shop;
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        ...
        <div className="flex gap-2 shrink-0">
          <div className="flex items-center gap-1.5 border border-black px-2 py-1.5">
            <span className="text-[10px] font-bold tracking-widest uppercase text-neutral-500 shrink-0">From</span>
            <input
              type="date"
              value={fromDate}
          ...
          <div className="flex items-center gap-1.5 border border-black px-2 py-1.5">
            <span className="text-[10px] font-bold tracking-widest uppercase text-neutral-500 shrink-0">To</span>
            <input
              type="date"
              value={toDate}
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-black bg-neutral-50">
                  {['Bill No.', 'Vehicle', 'Shop', 'Date', 'Net (KG)', ''].map((h) => (
                    <th
                      key={h}
                      className="text-left px-3 md:px-4 py-3 font-bold tracking-widest uppercase whitespace-nowrap"
                    >
                      {h}
                    </th>
                  ))}
              ...
              <tbody>
                {paginated.map((bill) => (
                  ...
                  >
                    <td className="px-3 md:px-4 py-3 font-bold">{bill.billNo}</td>
          ...
          <div className="space-y-4 text-xs font-mono">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(v) => ef('material', v.toUpperCase())}
              />
            </div>
      ...
      {deleteBill && (
          ...
          footer={
            <>
              <button
                ...
                disabled={deleting}
                className="flex items-center gap-2 bg-red-600 text-white text-xs px-4 py-2 tracking-widest uppercase hover:bg-red-700 disabled:opacity-50"
              >
                <Trash2 size={12} />
        ...
        >
          <p className="text-xs font-mono text-neutral-700 leading-relaxed">
            ...
            <span className="font-bold">Bill #{deleteBill.billNo}</span>
            {deleteBill.vehicleNo ? (
              <> — {deleteBill.vehicleNo}</>
            ) : null}
...
function Label({ children }: { children: React.ReactNode }) {
  return (
    <label className="block font-bold tracking-widest uppercase mb-1 text-[10px]">
      {children}
    </label>
  );
}
...
function PaginationBtn({
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={`
        min-w-[26px] h-[26px] flex items-center justify-center text-xs border font-mono
        ${active ? 'bg-black text-white border-black' : 'border-black hover:bg-neutral-100'}
